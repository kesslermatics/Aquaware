...
class OrganicCarbonDataScreen extends StatefulWidget {
  final int aquariumId;

  OrganicCarbonDataScreen({required this.aquariumId});

  @override
  _OrganicCarbonDataScreenState createState() =>
      _OrganicCarbonDataScreenState();
}

class _OrganicCarbonDataScreenState extends State<OrganicCarbonDataScreen> {
  ...
  Future<List<WaterValue>> _fetchWaterValues() async {
    return await _waterParameterService.fetchSingleWaterParameter(
        widget.aquariumId, 'Organic Carbon',
        numberOfEntries: 100);
  }
  ...
  Widget _makeLineChartWidget(List<WaterValue> waterValues) {
    ...
    double yDeviation = 2.0; // Example deviation for Organic Carbon
    return LineChartWidget(
      xValues: xValues,
      yValues: yValues,
      yDeviation: yDeviation,
      fractionDigits: 1,
      title: 'Organic Carbon Levels over Time',
      xAxisLabel: 'Time',
      yAxisLabel: 'Organic Carbon (mg/L)',
    );
  }
  ...
  Widget _makeHeatmapWidget(List<WaterValue> waterValues) {
    return HeatmapWidget(
      ...
      fractionDigits: 1,
      title: "Organic Carbon Level Heatmap",
    );
  }
}
