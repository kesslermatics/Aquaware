...
class SuspendedSolidsDataScreen extends StatefulWidget {
  final int aquariumId;

  SuspendedSolidsDataScreen({required this.aquariumId});

  @override
  _SuspendedSolidsDataScreenState createState() =>
      _SuspendedSolidsDataScreenState();
}

class _SuspendedSolidsDataScreenState extends State<SuspendedSolidsDataScreen> {
  ...
  Future<List<WaterValue>> _fetchWaterValues() async {
    return await _waterParameterService.fetchSingleWaterParameter(
        widget.aquariumId, 'Suspended Solids',
        numberOfEntries: 100);
  }
  ...
  Widget _makeLineChartWidget(List<WaterValue> waterValues) {
    ...
    double yDeviation = 5.0; // Example deviation for Suspended Solids
    return LineChartWidget(
      xValues: xValues,
      yValues: yValues,
      yDeviation: yDeviation,
      fractionDigits: 1,
      title: 'Suspended Solids Levels over Time',
      xAxisLabel: 'Time',
      yAxisLabel: 'Suspended Solids (mg/L)',
    );
  }
  ...
  Widget _makeHeatmapWidget(List<WaterValue> waterValues) {
    return HeatmapWidget(
      ...
      fractionDigits: 1,
      title: "Suspended Solids Heatmap",
    );
  }
}
