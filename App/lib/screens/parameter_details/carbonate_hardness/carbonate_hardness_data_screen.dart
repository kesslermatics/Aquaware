...
class CarbonateHardnessDataScreen extends StatefulWidget {
  final int aquariumId;

  CarbonateHardnessDataScreen({required this.aquariumId});

  @override
  _CarbonateHardnessDataScreenState createState() =>
      _CarbonateHardnessDataScreenState();
}

class _CarbonateHardnessDataScreenState
    extends State<CarbonateHardnessDataScreen> {
  ...
  Future<List<WaterValue>> _fetchWaterValues() async {
    return await _waterParameterService.fetchSingleWaterParameter(
        widget.aquariumId, 'Carbonate Hardness',
        numberOfEntries: 100);
  }
  ...
  Widget _makeLineChartWidget(List<WaterValue> waterValues) {
    ...
    double yDeviation = 2.0; // Example deviation for Carbonate Hardness
    return LineChartWidget(
      xValues: xValues,
      yValues: yValues,
      yDeviation: yDeviation,
      fractionDigits: 1,
      title: 'Carbonate Hardness over Time',
      xAxisLabel: 'Time',
      yAxisLabel: 'Carbonate Hardness (°dH)',
    );
  }
  ...
  Widget _makeHeatmapWidget(List<WaterValue> waterValues) {
    return HeatmapWidget(
      ...
      fractionDigits: 1,
      title: "Carbonate Hardness Heatmap",
    );
  }
}
