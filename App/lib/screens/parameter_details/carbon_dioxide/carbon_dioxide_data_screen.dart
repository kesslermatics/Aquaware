...
class CarbonDioxideDataScreen extends StatefulWidget {
  final int aquariumId;

  CarbonDioxideDataScreen({required this.aquariumId});

  @override
  _CarbonDioxideDataScreenState createState() =>
      _CarbonDioxideDataScreenState();
}

class _CarbonDioxideDataScreenState extends State<CarbonDioxideDataScreen> {
  ...
  Future<List<WaterValue>> _fetchWaterValues() async {
    return await _waterParameterService.fetchSingleWaterParameter(
        widget.aquariumId, 'Carbon Dioxide',
        numberOfEntries: 100);
  }
  ...
  Widget _makeLineChartWidget(List<WaterValue> waterValues) {
    ...
    double yDeviation = 5.0; // Example deviation for Carbon Dioxide
    return LineChartWidget(
      ...
      xAxisLabel: 'Time',
      yAxisLabel: 'Carbon Dioxide (mg/L)',
    );
  }
  ...
  Widget _makeHeatmapWidget(List<WaterValue> waterValues) {
    return HeatmapWidget(
      ...
      fractionDigits: 2,
      title: "Carbon Dioxide Level Heatmap",
    );
  }
}
