...
class ConductivityDataScreen extends StatefulWidget {
  final int aquariumId;

  ConductivityDataScreen({required this.aquariumId});

  @override
  _ConductivityDataScreenState createState() => _ConductivityDataScreenState();
}

class _ConductivityDataScreenState extends State<ConductivityDataScreen> {
  ...
  Future<List<WaterValue>> _fetchWaterValues() async {
    return await _waterParameterService.fetchSingleWaterParameter(
        widget.aquariumId, 'Conductivity',
        numberOfEntries: 100);
  }
  ...
  Widget _makeLineChartWidget(List<WaterValue> waterValues) {
    ...
    double yDeviation = 100.0; // Example deviation for Conductivity
