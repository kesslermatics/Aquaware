...
class RedoxPotentialDataScreen extends StatefulWidget {
  final int aquariumId;

  RedoxPotentialDataScreen({required this.aquariumId});

  @override
  _RedoxPotentialDataScreenState createState() =>
      _RedoxPotentialDataScreenState();
}

class _RedoxPotentialDataScreenState extends State<RedoxPotentialDataScreen> {
  ...
  Future<List<WaterValue>> _fetchWaterValues() async {
    return await _waterParameterService.fetchSingleWaterParameter(
        widget.aquariumId, 'Redox Potential',
        numberOfEntries: 100);
  }
  ...
  Widget _makeLineChartWidget(List<WaterValue> waterValues) {
    ...
    double yDeviation = 50.0; // Example deviation for Redox Potential
    return LineChartWidget(
      xValues: xValues,
      yValues: yValues,
      yDeviation: yDeviation,
      fractionDigits: 1,
      title: 'Redox Potential over Time',
      xAxisLabel: 'Time',
      yAxisLabel: 'Redox Potential (mV)',
    );
  }
